...
（，，。。测试）我们可以在github创建Maven存储库，这样可以在github托管java的依赖构件。如果有工程需要依赖，也可以通过maven方便的下载依赖。也许我们将Maven存储库托管在GitHub上有许多理由，包括许可问题、隐私和商业托管成本。无论您为什么选择托管基于GitHub的Maven存储库，设置一个存储库都很容易。具体设置步骤如下:
